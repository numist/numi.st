...
Like most lazy Jekyll sites the style is derived from minima, though I did go to [some lengths](https://github.com/numist/highlight-css) to find a good syntax highlighting theme.

Discrete features I've written (or at least adapted) for the site include:
...
I'd like the footnotes to live next to the text when the viewport is big enough—Tanya says I can probably do this using [`<div class="d-none d-lg-block">`](https://getbootstrap.com/docs/5.2/utilities/display/) so I guess I have to integrate (and learn) Bootstrap now.

I'd also like an excuse to publish some pages that use [Tangle](http://worrydream.com/Tangle/). I'll probably start by onshoring some online "calculators" that I find myself visiting often, like Sheldon Brown's [gear calculator](https://www.sheldonbrown.com/gear-calc.html) or Surplace's [skid patch calculator](https://www.surplace.fr/ffgc/).
...
All pages should probably have ctime and mtime somewhere, for posterity.
...
[^pagination]: Basically, pages infinite-scrolled by fetching the next page appending its `<article>`s to the current page. More importantly, it _updated the browser's address bar_ so reloading the page would cause all the currently visible content to be part of the first request, preserving the reader's scroll location.
[^newsfoot]: Under the assumption that it is available under the MIT license, based on its inclusion in [NetNewsWire's source code](https://github.com/Ranchero-Software/NetNewsWire/blob/57815f04960f08a78b0fe9972b6a9d8993103e61/Shared/Article%20Rendering/newsfoot.js).
